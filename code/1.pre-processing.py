...
argparser = argparse.ArgumentParser(description="Preprocessing for YOLOv5")
argparser.add_argument(
    "-r", "--rename", help="rename the image, label and mask files", action="store_true", default=False)
argparser.add_argument(
    "-s", "--save", help="save the image, label and mask files", action="store_true", default=False)
argparser.add_argument(
    "-c", "--copy", help="copy the image, label and mask files", action="store_true", default=False)
...
#
yolo_train_image_path = "..\\yolo\\defect\\images\\train\\"
yolo_val_image_path = "..\\yolo\\defect\\images\\val\\"
yolo_image_path = "..\\yolo\\defect\\"
...
train_set_dir = '..\\class_data\\Train'
test_set_dir = '..\\class_data\\Val'
...
def yolo_format(convert_img_file: str, save_img_file_name: str, save: bool = False):
    ...
    for annotation in json_file["shapes"]:
        ...
        if args.save == True:
            with open(train_set_dir.replace("images", "labels").replace("train", save_img_file_name) + image_name.replace(".png", ".txt"), "a") as file:
                print(train_set_dir.replace("images", "labels").replace("train", save_img_file_name) + image_name.replace(".png", ".txt"))
        ...
        for class_name in class_names:
            ...
            for type_name in types:
                ...
                for filename in tqdm(os.listdir(type_dir)):
                    ...
                    if args.rename == True:
                        print(f"rename {filename} to {class_name}_{filename.replace(' ', '').lower()}")
                        # rename the image name with class + image name
                        os.rename(
            ...
            for type_name in types:
                ...
                for filename in tqdm(os.listdir(type_dir)):
                    ...
                    if dataset == train_set_dir:
                        yolo_format(
                            convert_img_file=f"{type_dir}\\{filename}", save_img_file_name='train', save=args.save)
                    else:
                        yolo_format(
                            convert_img_file=f"{type_dir}\\{filename}", save_img_file_name='val', save=args.save)
